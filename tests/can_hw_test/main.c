 ...
 * 				Javier Fileiv <javier.fileiv@continental-corporation.com>
 *
 * @}
 */
#include <string.h>
...
static int _send(int argc, char **argv)
{
    ...
    if (argc < 5) {
        print_usage();
        puts("Error: Invalid parameter length");
        return 1;
    }
    struct can_frame frame;
    int ifnum = strtol(argv[2], NULL, 0);
    if (ifnum >= CAN_DLL_NUMOF) {
        puts("Error: Invalid interface number");
        return 1;
    }
...
static int _receive(int argc, char **argv)
{
    ...
    if (thread_nb >= RCV_THREAD_NUMOF) {
        printf("Error: Invalid thread number, range=0..%d\n", RCV_THREAD_NUMOF - 1);
        return 1;
    }
...
{
    puts("Command: test_can close user_id");
    ...
    if(!conn_can_raw_close(&conn[thread_nb])){
    	puts("Success: Close OK\n");
    }
...
{
    puts("Command: test_can bind_isotp ifnum user_id source_id dest_id");
...
{
    puts("Command: test_can send_isotp user_id [B1 [.. [ Bn ]]]");
...
{
    puts("Command: test_can recv_isotp user_id timeout");
...
{
    puts("Command: test_can close_isotp user_id");
...
{
	puts("Command: test_can get_filter ifnum");
...
{
	puts("Command: test_can set_bitrate ifnum bitrate [sample_point]");
...
{
	puts("Command: test_can get_bitrate ifnum");
...
{
	puts("Command: test_can get_counter ifnum");
...
{
	puts("Command: test_can power_up ifnum");
...
{
	puts("Command: test_can power_down ifnum");
